  ...
  for line in song_lyrics.splitlines():
    ...
    if len_tokens > 0:
      for i in xrange(len_tokens):
        ...
        if i < len_tokens - 1: # Not the last token of the line: count bigram
...
def compute_frequencies(lyrics_data):
  ...
  Given a data dictionary of the form returned by collect_data, returns a data
  dictionary with the same keys, but counts converted to probabilities.
  '''
...
def smart_uncapitalize(string):
  ...
  Also avoid uncapitalizing a first person subject pronoun (e.g. I, I'm, etc.).
  '''
